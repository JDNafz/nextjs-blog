...
export const getUsersWithPasswords = async (): Promise<Omit<User, "password">[]> => {
	const { rows } = await query<User>('SELECT * FROM "user"');
...
export const getUserById = async (id: number): Promise<User> => {
	const { rows } = await query<User>('SELECT * FROM "user" WHERE id = $1', [id]);
	return rows[0];
}
...
export const createUser = async (newUser: Omit<User, "id">): Promise<User | null> => {
	...
	const { rows } = await query<User>(`INSERT INTO "user" (name,   	email, password)
    VALUES ($1, $2, $3)
    RETURNING *`,
		[name, email, password]
